...
"""
run with:
spark-submit ./irc1-ex3.1.py local[2] 10.0.0.1 9999
"""
import sys
...
country = words.map(lambda x: (x['cha'], 1))

countrywindow = country.window(30,10)
byCountry = countrywindow.reduceByKey(lambda x, y: x + y)
# save the dictionary to hdfs by specifing a directory name perfix
minByCountry = byCountry.reduce(find_min)
# save the dictionary to hdfs by specifing a directory name perfix
minByCountry.saveAsTextFiles("hdfs:///tmp/irc-data-countwindow")
